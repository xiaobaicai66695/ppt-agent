...
- 每次部署前必须在确认目标路径为 `/ppt/ppt-agent` 且已停止旧进程后，完整删除本次交付的远程服务端代码目录（当前为 `backend` 的源码内容及 `skills`），再上传当前提交的最新服务端代码；禁止增量覆盖旧源码或保留旧服务端文件。`backend/.env` 和 `weboutput` 分别是运行配置与用户生成结果，不属于可删除的代码，必须保留。删除后确认代码目录仅包含本次交付内容，随后再构建、重启和冒烟。
- 每次部署前必须在确认目标路径为 `/ppt/ppt-agent` 且已停止旧进程后，完整删除该远程服务端源码目录，再上传当前提交的最新服务端代码；禁止增量覆盖旧源码或保留旧服务端文件。删除后必须确认新目录仅包含本次交付内容，随后再构建、重启和冒烟。
