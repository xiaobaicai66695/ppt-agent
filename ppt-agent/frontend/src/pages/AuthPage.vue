...
import { ArrowLeft, ArrowRight, Check, Eye, EyeOff, FileStack, KeyRound, LockKeyhole, Mail, UserRound } from 'lucide-vue-next'
  ...
  <main class="auth-page">
    ...
    <section class="auth-panel" :aria-labelledby="isRegister ? 'register-title' : 'login-title'">
      ...
      <form novalidate @submit.prevent="submit">
        ...
        <div v-if="isRegister || loginMode === 'password'" class="field"><label for="auth-password">{{ isRegister ? '设置密码' : '密码' }}</label><div class="input-wrap"><LockKeyhole :size="16" aria-hidden="true" /><input id="auth-password" v-model="password" :type="passwordVisible ? 'text' : 'password'" :autocomplete="isRegister ? 'new-password' : 'current-password'" :placeholder="isRegister ? '8–64 位，含大小写字母和数字' : '输入密码'" required /><button type="button" class="password-toggle" :aria-label="passwordVisible ? '隐藏密码' : '显示密码'" :aria-pressed="passwordVisible" @click="passwordVisible = !passwordVisible"><EyeOff v-if="passwordVisible" :size="17" aria-hidden="true" /><Eye v-else :size="17" aria-hidden="true" /></button></div><p v-if="isRegister" class="field-note" :class="{ success: passwordRule }"><Check v-if="passwordRule" :size="13" />密码需为 8–64 位，并包含大写字母、小写字母和数字。</p></div>
        <div v-if="isRegister" class="field"><label for="auth-confirm-password">确认密码</label><div class="input-wrap"><LockKeyhole :size="16" aria-hidden="true" /><input id="auth-confirm-password" v-model="confirmPassword" :type="confirmPasswordVisible ? 'text' : 'password'" autocomplete="new-password" placeholder="再次输入密码" required :aria-invalid="confirmPassword.length > 0 && confirmPassword !== password" /><button type="button" class="password-toggle" :aria-label="confirmPasswordVisible ? '隐藏确认密码' : '显示确认密码'" :aria-pressed="confirmPasswordVisible" @click="confirmPasswordVisible = !confirmPasswordVisible"><EyeOff v-if="confirmPasswordVisible" :size="17" aria-hidden="true" /><Eye v-else :size="17" aria-hidden="true" /></button></div><p v-if="confirmPassword.length > 0 && confirmPassword !== password" class="field-note error">两次输入的密码不一致。</p></div>
