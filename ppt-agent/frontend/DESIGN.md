...
`AppShell` 负责固定侧栏和主工作区；活跃 Dashboard 在主画布中按到达顺序排列“任务上下文 → 用户消息 → 思考摘要 → 工具调用/返回卡 → 下一段思考或最终回答 → 交付 → 输入”。一次工具调用只占一张成对卡片：调用到达时立即出现，返回到达时在同一张卡内更新状态与返回内容，不能另起底部结果堆栈，也不能复制出第二张同名工具卡。思考摘要与工具调用/返回卡可收起，收起不删除状态；长返回内容不截断，放在卡内可滚动的小结果区。本次页面会话到达终态后仍保留可观察轨迹，冷启动的持久化会话只恢复用户消息与最终助手回答。`answer_end` / `llm_end` 只结束当前文本段，不改变任务的运行态或关闭 SSE；下一次 `llm_start` 后即使复用同一个 `segment_id`，也必须创建新的文本卡。窄屏隐藏任务侧栏，主画布保留输入与停止操作。
