...
`AppShell` 负责固定侧栏和主工作区；宽屏 Dashboard 以“会话列表 → 对话与执行主画布 → 交付预览”排列，已完成 PPT 的缩略图在独立右侧栏中持续可见，不混入消息时间线。一次工具调用仍是调用与返回成对的可核验记录；从 `llm_end` 到下一次 `llm_start` 的全部工具记录则归入一张默认折叠的“工具调用（N 项）”卡。旧历史记录没有这两个边界时，连续的工具记录同样归入一张默认折叠卡。展开后按原顺序展示每项完整内容，避免每次工具循环打断主时间线。已脱敏的调用参数和返回内容不截断，放在卡内最多约 196px 高、可纵向滚动的小文本区。会话消息与已向用户展示的安全轨迹共同持久化，历史任务必须按原时间顺序恢复；旧任务没有轨迹表记录时，至少从已保存的运行事件恢复工具调用/结果与阶段摘要。`answer_end` / `llm_end` 只结束当前文本段，不改变任务的运行态或关闭 SSE；下一次 `llm_start` 后即使复用同一个 `segment_id`，也必须创建新的文本卡。中等宽度先隐藏会话列表；窄屏将交付栏顺序下移，主画布保留输入与停止操作。
