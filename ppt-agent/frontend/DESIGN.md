...
`AppShell` 负责固定侧栏和主工作区；宽屏 Dashboard 以“会话列表 → 对话与执行主画布 → 交付预览”排列，已完成 PPT 的缩略图在独立右侧栏中持续可见，不混入消息时间线。一次工具调用只占一张成对卡片：调用到达时立即出现，返回到达时在同一张卡内更新状态与返回内容，不能另起底部结果堆栈，也不能复制出第二张同名工具卡。思考摘要与工具调用/返回卡可收起，收起不删除状态；长返回内容不截断，放在卡内可滚动的小结果区。会话消息与已向用户展示的安全轨迹共同持久化，历史任务必须按原时间顺序恢复；旧任务没有轨迹表记录时，至少从已保存的运行事件恢复工具调用/结果与阶段摘要。`answer_end` / `llm_end` 只结束当前文本段，不改变任务的运行态或关闭 SSE；下一次 `llm_start` 后即使复用同一个 `segment_id`，也必须创建新的文本卡。中等宽度先隐藏会话列表；窄屏将交付栏顺序下移，主画布保留输入与停止操作。
