...
图片评分口径：当 `model_output` 中图片计划为 `search_status="planned"` 且包含可执行英文 `asset_query`、明确 `asset_subject` 和合理 `composition` 时，不因缺少 `local_path/source_url/attribution` 扣分；**同一 Deck 内相同 `content_type` 的背景必须规划为同一共享 `asset_query`，不得按页轮换背景。** 只有伪造下载结果、完全缺少视觉策略、同类型背景查询不一致、图片计划与主题无关或把下载失败伪装成成功时才扣分。
