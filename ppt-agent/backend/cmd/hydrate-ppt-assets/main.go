...
func main() {
	...
	manifest, err := ppt.ReadTasksManifest(absWorkDir)
	if err != nil {
		fatal(err.Error())
	}
	counts, err := ppt.MaterializePlannedPPTAssets(context.Background(), absWorkDir, manifest)
	if err != nil {
		fatal(err.Error())
	}
	if counts.Backgrounds > 0 || counts.Images > 0 {
		if err := ppt.WriteTasksManifest(absWorkDir, manifest); err != nil {
			fatal(err.Error())
		}
	}
