...
type TaskItem struct {
	...
	Background    string       `json:"background,omitempty"`   // legacy local background field; new plans keep it empty
}
...
type TaskOutline struct {
	...
	} `json:"background_options,omitempty"` // legacy local background options; new plans use image search intent
}
...
func (e *ContentElement) UnmarshalJSON(data []byte) error {
	...
	if err != nil {
		return fmt.Errorf("content_plan element text: %w", err)
	}
	description, err := decodeContentElementText(raw.Description)
	if err != nil {
		return fmt.Errorf("content_plan element description: %w", err)
	}
	*e = ContentElement{
		Type: raw.Type, Items: items, Text: text, Title: raw.Title,
		Description: description, LayoutHint: raw.LayoutHint,
	}
	return nil
}

func decodeContentElementText(data json.RawMessage) (string, error) {
