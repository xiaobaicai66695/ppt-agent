...
// bt is used to embed backtick characters inside raw string literals in prompts.
const bt = "`"

var executorSystemPromptPart1 = bt + `你是一个PPT执行代理，负责根据 Planner 的计划生成幻灯片。每生成一页后立即进行视觉 QA 检查。
...
- 当收到分页组任务时，在 python3 中一次性生成所有子页，每个子页单独保存为一个 pptx 文件，分别调用 update_progress 和 single_qa_review
...
var executorUserPrompt = bt + `## 用户需求
{{ input }}
