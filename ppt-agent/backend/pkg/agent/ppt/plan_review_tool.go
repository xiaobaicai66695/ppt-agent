...
const minVisualMixedSlidesForPPT = 2
...
func reviewPPTBackgroundVariety(manifest *TasksManifest, report *PlanReviewReport) {
...
func reviewPPTVisualMix(manifest *TasksManifest, report *PlanReviewReport) {
	if manifest == nil || report == nil {
		return
	}
	contentSlides := 0
	...
	if contentSlides >= 4 && visualMixedSlides < minVisualMixedSlidesForPPT {
		report.Issues = append(report.Issues, PlanReviewIssue{
			Code:     "layout_mismatch",
			Severity: "warning",
...
func RequirePassingPlanReview(workDir string, manifest *TasksManifest) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("draft PPTSpec has not been reviewed by the Go review workflow")
		}
		return fmt.Errorf("read plan review report: %w", err)
	}
	if !report.Passed {
		return fmt.Errorf("latest PPTSpec review did not pass: %s", report.Summary)
	}
	fingerprint := fingerprintTasksManifest(manifest)
	if report.Fingerprint != fingerprint {
		return fmt.Errorf("PPTSpec changed after latest review; run the Go review workflow again before commit")
	}
	return nil
}
...
func informationNarrativeChars(components []PlanComponent) int {
	...
	for _, component := range components {
		...
		switch strings.TrimSpace(component.Type) {
		case "headline", "ppt_title", "subheadline", "section_marker", "source_note", "image", "map", "diagram", "divider", "shape", "arrow", "icon", "tag":
			continue
		case "list", "numbered_list", "bullet_list", "evidence_list":
