...
func nextWithTimeout(ctx context.Context, iter *adk.AsyncIterator[*adk.AgentEvent]) (*adk.AgentEvent, bool, bool) {
	...
	case <-time.After(timeout):
		// Cancel the runner context as well as returning to the caller. This
		// closes the producer side of the iterator and lets the waiter goroutine
...
type iteratorCancelKey struct{}

func cancelIterator(ctx context.Context) {
	if cancel, ok := ctx.Value(iteratorCancelKey{}).(context.CancelFunc); ok && cancel != nil {
		cancel()
	}
}
...
func ResumePPTPlannerFromDraftWithCallback(ctx context.Context, cfg *PPTTaskConfig,
	...
	if cfg == nil || strings.TrimSpace(cfg.WorkDir) == "" {
		return nil, fmt.Errorf("恢复 PPTSpec 失败：任务工作目录为空")
	}
	...
	if manifest == nil || len(manifest.Tasks) == 0 {
		return nil, fmt.Errorf("恢复后 PPTSpec 为空，无法进入渲染")
	}
...
func runPPTPlannerInternal(ctx context.Context, agent adk.Agent, cfg *PPTTaskConfig,
	...
	defer cancelRun()
	runCtx = context.WithValue(runCtx, iteratorCancelKey{}, cancelRun)
	start, err := StartPPTPlanner(runCtx, agent, cfg, userQuery)
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		event, ok, timedOut := nextWithTimeout(runCtx, iter)
		if runCtx.Err() != nil {
			if lastMessageStream != nil {
				lastMessageStream.Close()
			}
	...
	if manifest == nil || len(manifest.Tasks) == 0 {
		return result, fmt.Errorf("Planner 生成的 PPTSpec 为空，无法进入渲染")
	}
	result.TotalSlides = len(manifest.Tasks)
	result.DoneSlides = manifest.CompletedCount()
	for _, t := range manifest.Tasks {
...
func ensurePlannerDraft(cfg *PPTTaskConfig, userQuery, plannerOutput string, onEvent AgentEventCallback) (*TasksManifest, error) {
	if draft, err := ReadTasksDraftManifest(cfg.WorkDir); err == nil && draft != nil && len(draft.Tasks) > 0 {
		...
		return nil, fmt.Errorf("读取 Planner PPTSpec 草稿失败: %w", err)
	}
	...
	if existing, err := ReadTasksManifest(cfg.WorkDir); err == nil && existing != nil && len(existing.Tasks) > 0 {
		if err := WriteTasksDraftManifest(cfg.WorkDir, existing); err != nil {
			return nil, fmt.Errorf("迁移现有 PPTSpec 到审查草稿失败: %w", err)
		}
		return existing, nil
	} else if err != nil && !os.IsNotExist(err) {
		return nil, fmt.Errorf("读取现有 PPTSpec 失败: %w", err)
	}

	recovered, err := recoverMissingPlannerManifest(cfg, userQuery, plannerOutput)
	if err != nil {
		return nil, fmt.Errorf("Planner 未生成 PPTSpec 草稿，代码恢复失败: %w", err)
	}
	onEvent(AgentEvent{
		Type:    AgentEventAnswer,
	...
	if cfg.RuntimeMeta != nil {
		cfg.RuntimeMeta.RecordEvent("ppt_spec_recovered", tasksDraftFileName, "warning", fmt.Sprintf("recovered %d slides from planner output", len(recovered.Tasks)), map[string]any{
...
func reviewAndCommitPPTSpec(ctx context.Context, cfg *PPTTaskConfig, onEvent AgentEventCallback) (*TasksManifest, error) {
	maxPlanReviewRounds := retry.Default().MaxAttempts(retry.OperationPPTSpecReview)
	if maxPlanReviewRounds < 1 {
		return nil, fmt.Errorf("未配置 PPTSpec 审查重试策略")
	}
	...
	if startRound > maxPlanReviewRounds {
		return nil, fmt.Errorf("PPTSpec 审查检查点轮次 %d 超出最大轮次 %d", startRound, maxPlanReviewRounds)
	}
	...
	for round := startRound; round <= maxPlanReviewRounds; round++ {
		...
		if cfg.RuntimeMeta != nil {
			...
			cfg.RuntimeMeta.RecordEvent("ppt_spec_review", "TaskPlanReviewer", status, report.Summary, map[string]any{
				"round":       round,
		...
		if report.Passed {
			...
			if !ok || manifest == nil {
				return nil, fmt.Errorf("PPTSpec 审查通过但没有可提交的规划草稿")
			}
			if err := clearPlanReviewCheckpoint(cfg.WorkDir); err != nil {
				return nil, err
			}
			onEvent(AgentEvent{Type: AgentEventAnswer, Content: fmt.Sprintf("PPTSpec 已通过 Task Reviewer 并提交，共 %d 页，开始进入并发渲染。\n", len(manifest.Tasks))})
			if cfg.RuntimeMeta != nil {
				cfg.RuntimeMeta.RecordEvent("ppt_spec_committed", tasksDraftFileName, "success", fmt.Sprintf("committed %d slides from reviewed draft", len(manifest.Tasks)), map[string]any{
	...
	return nil, fmt.Errorf("PPTSpec 在 %d 轮审查后仍未通过：%s", maxPlanReviewRounds, latest.Summary)
}
...
func readPlanReviewCheckpoint(workDir string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("读取 PPTSpec 审查检查点失败: %w", err)
	}
	var checkpoint reviewCheckpoint
	if err := json.Unmarshal(data, &checkpoint); err != nil {
		return 0, fmt.Errorf("解析 PPTSpec 审查检查点失败: %w", err)
	}
	if checkpoint.NextRound < 1 {
		return 0, fmt.Errorf("PPTSpec 审查检查点轮次无效")
	}
...
func writePlanReviewCheckpoint(workDir string, nextRound int) error {
	if nextRound < 1 {
		return fmt.Errorf("PPTSpec 审查检查点轮次无效")
	}
	...
	if err := os.WriteFile(tmp, data, 0644); err != nil {
		return fmt.Errorf("写入 PPTSpec 审查检查点失败: %w", err)
	}
	if err := os.Rename(tmp, path); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("提交 PPTSpec 审查检查点失败: %w", err)
	}
	return nil
}
...
func clearPlanReviewCheckpoint(workDir string) error {
	if err := os.Remove(filepath.Join(workDir, reviewCheckpointFileName)); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("清理 PPTSpec 审查检查点失败: %w", err)
	}
	return nil
}
...
func runAgentWithCallback(ctx context.Context, agent adk.Agent, userInput string, onEvent AgentEventCallback) error {
	...
	err := streamAgentEvents(runCtx, iter, onEvent)
