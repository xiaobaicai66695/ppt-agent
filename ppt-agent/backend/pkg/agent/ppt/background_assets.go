...
type MaterializedPPTAssetCounts struct {
	Backgrounds int
	Images      int
}

// MaterializePlannedPPTAssets resolves all query-only external image plans
...
func MaterializePlannedPPTAssets(ctx context.Context, workDir string, manifest *TasksManifest) (MaterializedPPTAssetCounts, error) {
	backgroundTargets, err := collectPPTBackgroundTargets(workDir, manifest)
	if err != nil {
		return MaterializedPPTAssetCounts{}, err
	}
	...
	if len(backgroundTargets) == 0 && len(imageTargets) == 0 {
		return MaterializedPPTAssetCounts{}, nil
	}
	...
	return MaterializedPPTAssetCounts{Backgrounds: backgroundCount, Images: imageCount}, nil
}
...
func needsAssetClient(workDir string, backgroundTargets []plannedBackgroundTarget, imageTargets []plannedImageAssetTarget) bool {
	...
	assigned := assignPPTBackgroundRotation(append([]plannedBackgroundTarget(nil), backgroundTargets...))
...
// after every required search/download succeeds, avoiding partial PPTSpecs.
...
func collectPPTBackgroundTargets(workDir string, manifest *TasksManifest) ([]plannedBackgroundTarget, error) {
	if manifest == nil {
		return nil, fmt.Errorf("nil tasks manifest")
	}
	workDir = strings.TrimSpace(workDir)
...
func materializePlannedBackgroundsWithClient(ctx context.Context, workDir string, targets []plannedBackgroundTarget, client backgroundAssetClient) (int, error) {
	...
	targets = assignPPTBackgroundRotation(targets)
...
func assignPPTBackgroundRotation(targets []plannedBackgroundTarget) []plannedBackgroundTarget {
	if len(targets) == 0 {
		return targets
	}
...
func compactBackgroundSearchKeyword(query string) string {
	...
	stop := map[string]bool{
		...
		"slide": true, "ppt": true, "presentation": true,
	}
