...
func TestMaterializePlannedPPTAssetsDownloadsForegroundImageComponents(t *testing.T) {
	workDir := t.TempDir()
	manifest := &TasksManifest{Tasks: []*TaskItem{{
		TaskID: "page-1",
...
func TestMaterializePlannedPPTAssetsCollapsesExistingBackgroundsWithoutClient(t *testing.T) {
	workDir := t.TempDir()
	...
	counts, err := MaterializePlannedPPTAssets(context.Background(), workDir, manifest)
	if err != nil {
		t.Fatal(err)
	}
