...
def generate(prs: Optional[Presentation] = None, palette: str = "ocean_soft", source: str = "", title: str = "", subtitle: str = "", nodes: list[dict] = None, steps: list[dict] = None, direction: str = "", kicker: str = "", background: str = None, glass_colors: dict = None, **kwargs: Any) -> Presentation:
    ...
    components = [component("process_step", title=item.get("year") or item.get("num") or item.get("title"), body=item.get("event") or item.get("desc") or item.get("body")) for item in raw]
    components = prefer_explicit_components(kwargs, components)
    return render_component_slide(prs, palette, source, title, subtitle, kicker, components, "process_flow", direction, background)
