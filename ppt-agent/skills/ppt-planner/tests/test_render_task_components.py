...
class RenderTaskComponentsTest(unittest.TestCase):
    ...
    def test_all_background_slides_receive_blur(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(background_mock.call_args_list[0].kwargs["blur_radius"], 6)
            self.assertEqual(background_mock.call_args_list[1].kwargs["blur_radius"], 12)
    ...
    def test_atomic_components_render_without_coordinates(self):
        prs = render_component_slide(
            ...
            components=[
                ...
                {"type": "architecture_box", "title": "PPT Planner", "body": "负责整套叙事、章节和页面角色，不接触坐标、字号或颜色。", "role": "规划层"},
                {"type": "architecture_box", "title": "Component Planner", "body": "把每页拆成可执行语义组件，供生成器稳定消费。", "role": "规划层"},
                {"type": "arrow", "relation": "输出 PPTSpec", "target": "box_2"},
            ],
        )
