---
name: ppt-planner
description: 规划可执行的 PPTSpec/tasks.json，并用组件化 Python generators 校验和渲染 PPTX。
---
...
- 用户要求创建或修改一套 PPT、PPTX、演示文稿或 slide ppt。
- 需要把自然语言需求、大纲、研究材料或数据转成结构化 PPTSpec。
...
- 在本 skill 目录或 `ppt-agent` 项目中处理 PPT 请求时，本 skill 负责 PPTSpec、素材路线、校验和渲染；先读取本文件并完成素材预检，再调用通用 PPT 或图像生成能力。
...
Planner 应尽力一次性填写完整 PPTSpec 内容字段，包括：

- ppt 级标题、受众、目标、章节或内容素材；
...
背景不是按页轮换的装饰素材：每个 `content_type` 在一套 ppt 内只选定一张背景。若同类型页已有不同背景路径，必须在渲染前统一为该类型的一个已解析背景；`validate_ppt.py` 会拒绝这种不一致的任务清单。

需要由 Agent 搜图时，按以下边界执行：

- **通用 Agent**：必须使用本 skill `scripts/` 下的 Unsplash CLI，例如 Codex、Claude Code、OpenCode、Pi 等。CLI 负责搜索并下载可用于本次 PPTSpec 的本地素材。
...
```bash
python generators/validate_ppt.py --work-dir <work-dir> --skills-dir <skills-dir>
```

整套生成优先使用 `generators/render_ppt.py`：

```bash
python generators/render_ppt.py --work-dir <work-dir> --skills-dir <skills-dir> --output ppt.pptx
```

默认执行顺序为：`视觉规划 → 图片搜索与下载 → validate_ppt.py → render_ppt.py → PDF/PNG 视觉验收`。无图页只能在已记录豁免或搜索失败后执行预检和渲染。
...
- 常规页面生成必须复用 `generators` 包、`validate_ppt.py`、`render_ppt.py` 或 `render_task.py`，不要让 Agent 手写底层 `python-pptx` 绘图代码。
