#!/usr/bin/env python3
"""Render every task in tasks.json into one PPTX ppt."""
...
def main() -> int:
    ...
    ppt_planner_dir = skills_dir / "ppt-planner"
    sys.path.insert(0, str(ppt_planner_dir))

    manifest_path = work_dir / "tasks.json"
    contract_path = ppt_planner_dir / "templates" / "component_contracts.json"
    validation = validate_ppt.validate_manifest_file(manifest_path, contract_path, work_dir)
