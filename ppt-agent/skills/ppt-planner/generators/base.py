...
def set_image_background(
    ...
    slide._ppt_ppt_background_path = os.path.abspath(image_path)
    slide._ppt_ppt_background_image = img.copy()
...
def add_glass_panel(
    ...
    if getattr(slide, "_ppt_ppt_background_image", None) is not None:
...
def _panel_crop(slide, left: float, top: float, width: float, height: float) -> Image.Image | None:
    ...
    image = getattr(slide, "_ppt_ppt_background_image", None)
