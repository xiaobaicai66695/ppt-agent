#!/usr/bin/env python3
"""Validate visual asset planning and materialization before ppt rendering."""
        ...
        if mode == "required":
            ...
            if not task_has_materialized_background:
                add_error(
                    ...
                    "only a ppt with visual_policy.mode=none and an explicit user background decline may omit it",
                )
    ...
    for content_type, entries in backgrounds_by_content_type.items():
        ...
        for task_path, _ in entries:
            add_error(
                ...
                f"all {content_type} pages in one ppt must reuse one materialized background image",
            )
