...
本文档用于验证 `ppt-planner` 在脱离 `ppt-agent` 后端时是否仍能作为独立 skill 工作。
...
```bash
python -m pip install -r skills/ppt-planner/requirements.txt
```
...
```bash
python generators/validate_ppt.py --work-dir examples/minimal --skills-dir ..
```
...
```bash
python generators/render_ppt.py --work-dir examples/minimal --skills-dir .. --output ppt.pptx
```
  ...
  "tasks": [
    {
      ...
      "content_plan": {
        "summary": "用一页内容验证 PPTSpec 可以被 render_task.py 消费。",
        ...
        "components": [
          ...
          },
          {
            "type": "bullet_list",
            "items": [
              "Planner 负责把用户需求转成完整 PPTSpec 内容字段，不写运行态字段。",
...
- `<skill-parent-dir>` 是包含 `ppt-planner/` 的目录；例如在本仓库中是 `skills`。
