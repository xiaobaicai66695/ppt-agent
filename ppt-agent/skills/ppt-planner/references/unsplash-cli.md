...
本 CLI 是 `ppt-planner` skill 的**通用 Agent**图片搜索与下载入口。`ppt-agent` 项目内的闲聊 Agent 改用 `search_images` Tool 展示图片候选；PPTPlanner 不直接调用 CLI 或图片 Tool，只输出视觉意图，再由审查后的确定性素材物化层执行搜索、下载与路径回填。

## 图片工具优先级

当 PPTSpec 使用默认的 `visual_policy.mode="required"` 时，这个 CLI 是 Agent 的**默认图片搜索与下载入口**：先规划每页 `visual_intent`，再用 `unsplash fetch` 物化图片。不要因为宿主提供了 `imagegen`、绘图或截图工具就跳过搜索并自行生成背景图。
...
set +a
node /ppt/ppt-agent/skills/ppt-planner/scripts/unsplash.mjs auth --from-env
```
