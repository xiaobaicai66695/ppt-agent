...
- 无论用户是否提供大纲，都生成或补全一份完整 PPTSpec 草稿。
...
- 背景图片、图片检索、内容密度、组件容量和 `visual_intent` 规则统一维护在 `skills/ppt-planner/SKILL.md`，Planner 与 Reviewer 共用。
...
- Agent 构造与提示词装配：`ppt-agent/backend/pkg/agent/ppt/agent.go`
- Planner/Reviewer 编排与提交：`ppt-agent/backend/pkg/agent/ppt/run.go`
- Reviewer 硬校验：`ppt-agent/backend/pkg/agent/ppt/plan_review_tool.go`
- Reviewer/Fixer 限权工具：`ppt-agent/backend/pkg/agent/ppt/fixer_manifest_tool.go`
- 生成后继续请求：`ppt-agent/backend/pkg/web/handler.go`
- 共享图片与组件策略：`ppt-agent/skills/ppt-planner/SKILL.md`
