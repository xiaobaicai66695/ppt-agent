...
- 将 Planner 首稿的正文容量规则从项目专属 prompt 迁入独立 `ppt-planner` skill，使通用 Agent 与项目内 Planner 读取同一份合同。
- 把“主题是否聚焦、页面是否推进论证”纳入真实 Planner benchmark，避免只以合法 JSON、字数和页面类型判断质量。

## 实现

- `skills/ppt-planner/templates/component_contracts.json` 新增 `planning_rules.page_content_density`，集中声明信息页、图文页、完整论述、列表项与双栏对照的容量规则；`SKILL.md` 和生成器参考将其标为唯一数字来源。
...
- `cmd/pptbench` 新增 `content_quality` 证据：ppt 主张、逐页主张、缺失主张页、完全重复主张和连续同类叙事布局；Planner Judge rubric 增加必填维度“主题聚焦与跨页叙事连贯性”。两个 test Planner case 增加中心结论、论证链和连续布局期望。

## 验证

- `go test ./cmd/pptbench ./pkg/prompts ./pkg/agent/ppt ./pkg/web`：通过。
- `go test ./test/plan_benchmark -run TestGoldPPTSpecsPassReviewer -count 1`：5/5 通过。
...
- 已同步 `/ppt/ppt-agent/skills/ppt-planner` 与 `/ppt/skills/ppt-planner`；两个 `component_contracts.json` SHA-256 一致。远端上传暂存目录已删除。
