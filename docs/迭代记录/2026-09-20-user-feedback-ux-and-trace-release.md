...
- 部署时间：2026-09-20 14:34 CST。
- 新进程：`/ppt/ppt-agent-linux -mode web -addr :8080`，PID `3663197`。
...
- 原有 `ppt-agent` 为 transient systemd 单元，停止后单元定义消失，无法用 `systemctl start` 恢复；当前新进程已存活并通过健康检查，但仍需单独治理为持久化启动单元，以覆盖机器重启和进程崩溃后的自动恢复。
