...
- 首次生成的 DeepAgent 仅注册 SlideExecutor，永久跳过 Reviewer 和自动 Fixer；显式的交付后页面修改能力继续保留。
...
- 当前版本路由实测输出 `agent_type=deep pipeline=plan,generate concurrency=5`；当前进程启动后的日志未出现 QA、Reviewer、Fixer 或 `intent_llm_fallback`。
...
- `ppt-agent/backend/pkg/prompts/deep/master_instruction.tmpl`
