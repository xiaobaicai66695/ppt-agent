...
- `计划时间` 记录事项首次登记日期，统一使用 `YYYY-MM-DD`。
- `完成时间` 在事项进入 `done` 时回填；未完成统一写 `-`。
- 表格按 `计划时间` 升序维护，同日事项保持登记先后顺序。
- 新事项只能追加到表尾，不得插入或重排历史条目；后续状态、时间和产出变化在原行更新。
...
6. **闭环要检查**: 按改动范围选择最小有效验证。Go 后端使用相关包的 `go test` 和必要的 `go build ./...`；Vue 前端使用 `npm run build`；Python PPT 生成器至少执行相关文件的 `py_compile`，视觉改动按需生成 PPTX 并渲染检查；OpenSpec 事项执行 `openspec validate <change> --strict`。
