...
Long ppts currently ask the Planner to produce a full executable `tasks.json` in one model turn. As page count, background image planning and component contracts grow, later pages often collapse into generic filler because the model must spend too much attention on schema and engineering fields instead of page-specific evidence.
...
- Split initial ppt planning into a blueprint phase, parallel section planning shards, deterministic merge, and one final Task Reviewer quality gate.
- Add stable section/page metadata so later user feedback can target one page or one section without regenerating the full ppt.
...
- Backend Agent orchestration under `ppt-agent/backend/pkg/agent/ppt`.
