...
The current ppt workflow creates one `PPTPlanner` agent and asks it to initialize a full executable `tasks.draft.json`. That single call must read skill files, choose narrative structure, plan page content, plan visual/background assets and emit a large schema-heavy manifest. As ppt size grows, output quality degrades toward generic page filler, especially in later pages.
...
- ppt title, theme and template fallback
...
- [Risk] Some short ppts do not need chunking. → Keep a threshold so small ppts can use the existing monolithic path or a single shard.
...
1. Add blueprint/section metadata structs and merge helpers behind the existing ppt workflow.
